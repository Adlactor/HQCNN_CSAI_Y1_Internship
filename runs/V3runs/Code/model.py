...
class HQCNN(nn.Module):
    ...
    def __init__(self, patch_size=2, stride=2, padding=0, q=2, num_classes=10, backend=None, random_seed=None,
        ...
        self.bn1    = nn.BatchNorm1d(256)           # Normalize 256 features :contentReference[oaicite:3]{index=3}
        self.fc2    = nn.Linear(256, 128)
        self.bn2    = nn.BatchNorm1d(128)           # Normalize 128 features :contentReference[oaicite:4]{index=4}
        self.fc3    = nn.Linear(128, num_classes)
        self.relu   = nn.ReLU()
        self.dropout = nn.Dropout(p=0.3)            # Slightly lower dropout to retain capacity :contentReference[oaicite:5]{index=5}
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)                            # (B, 256)
        x = self.bn1(x)                            # BatchNorm1d on 256 features :contentReference[oaicite:8]{index=8}
        x = self.relu(x)
        x = self.dropout(x)

        x = self.fc2(x)                            # (B, 128)
        x = self.bn2(x)                            # BatchNorm1d on 128 features :contentReference[oaicite:9]{index=9}
        x = self.relu(x)
        x = self.dropout(x)
